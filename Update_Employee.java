...
class Update_Employee extends Add_Employee implements ActionListener{
    ...
    Update_Employee(String idaa){
        ...
        Image i1=img.getImage().getScaledInstance(900, 500,Image.SCALE_DEFAULT);
    ...
    void showData(String s){
        try{
        	ConFile con = new ConFile();
            String str = "select * from emp where id = '"+s+"'";
    ...
    public void actionPerformed(ActionEvent ae){
        if(ae.getSource()==b && i==1){
            try{
    			...
    			Connection con1=DriverManager.getConnection("jdbc:mysql://localhost:3306/Employee","root","root");
            	PreparedStatement p = con1.prepareStatement("update emp set name=?,fname=?,age=?,dob=?,add=?,phone=?,email=?,education=?,jobpost=?,aadhar=? where id=?;");
                p.setString(1, t1.getText());
                p.setString(2, t2.getText());
                p.setString(3, age);
                p.setString(4, date);
                p.setString(5, t3.getText());
                p.setString(6, t4.getText());
                p.setString(7, t5.getText());
                p.setString(8, t6.getText());
                p.setString(9, t7.getText());
                p.setString(10, t8.getText());
              //  p.setString(11, t9.getText());
                p.setString(11, id_emp);
